class Map
  ...
  def construct_matrix
    ...
    (0..@height).each do |y_val|
      ...
      (0 .. @width).each do |x_val|
        ...
        if final_row_bomb
          ...
          row_bomb[y_val -1] ||= @width + 1
          if x_val >= (row_bomb[y_val -1] - 1) && Map.bomb == ascii_char
            row_bomb[y_val] = final_row_bomb = x_val
          end
        end
  
        # ascii_char << (x_val + y_val).to_s if ascii_char == Map.safe <-- not useful ...requires too many checks
        matrix_row << ascii_char
        # MatrixCell.new(@width, x_val, y_val, ascii_char)
      end # end-width
    ...
    # determine if each column has a "final" bomb...
    col_bomb = {-1 => 0}
    final_row = []
    (0 .. @width).each do |x_val|
      final_col_bomb = nil
      (0 .. @height).each do |y_val|
        ...
        if final_col_bomb
          # puts "overwriting the @matrix!"
          @matrix[y_val][x_val] = Map.bomb
        else
          col_bomb[x_val -1] ||= @height + 1
          if y_val >= (col_bomb[x_val -1] - 1) && Map.bomb == ascii_char
            col_bomb[x_val] = final_col_bomb = y_val
          end
        end # if we've got a final...bmb
      end # end height-loop

      # final_row << final_col_bomb ? Map.bomb : Map.success
    end # end width-loop
    # max_cycle.times { @matrix << final_row } # nonsense!
    ...
    # puts "constructed: #{@matrix.inspect}"
  end
  ...
  def success(row,col)
    ...
    # @matrix[row][col] == Map.success || row > @height || col > @width
  end
  ...
  def avail?(row,col)
    ...
    # puts " workin w/ r#{row}/c#{col}: #{@matrix[row][col]}"
    success(row,col) || @matrix[row][col] == Map.safe
  end
